...
restore_mode() {
        state=$(cat "${state_dir}"/theme)
        case $state in
                dark) mode=light ;;
                light) mode=dark ;;
                *)
                        >&1 echo "Invalid state saved. Defaulting to dark"
                        mode=light
                        ;;
        esac
}
...
case $@ in
        ...
        *) >&2 echo "Invalid command: not one of [dark, mocha, light, latte, toggle, restore] and not empty" ;;
esac
...
ln -sf "${conf_dir}"/imv/${new_mode} "${conf_dir}"/imv/config &
